...
import os
import sys
from typing import Dict, List, Optional, Tuple, Any, Callable # Added Callable for validator func type hint
...
def setup_logger(debug: bool = False, log_file_path: Optional[str] = None, log_level=logging.INFO):
    ...
    # return root_logger # Not strictly necessary to return if configuring root
...
def load_config(file_path: str, logger_instance: logging.Logger) -> Optional[Dict]:
    ...
    Returns:
        A dictionary containing the configuration data, or None if an error occurred.
    """
...
def get_config_section(config_data: Optional[Dict], section_name: str, default_value: Any = None) -> Any:
    ...
    Args:
        config_data: The configuration dictionary (can be None).
        section_name: The name of the section (key) to retrieve.
        default_value: The value to return if config_data is None or section is missing.

    Returns:
        The value of the section, or the default_value.
    """
    ...
    if section_data is default_value and section_name in config_data: # Key exists but value is the default (e.g. None)
...
def parse_devices_input(devices_input: Any) -> List[str]:
    ...
    if isinstance(devices_input, str):
        if devices_input.lower().startswith("file:"):
            ...
            # Split by comma, then strip whitespace, handle potential extra spaces
            identifiers = [item.strip() for item in devices_input.replace(' ', ',').split(',') if item.strip()]
    elif isinstance(devices_input, list):
        logger.debug(f"Parsing devices from list input ({len(devices_input)} items).")
        identifiers = [str(item).strip() for item in devices_input if isinstance(item, (str, int, float)) and str(item).strip()]
        if len(identifiers) < len(devices_input):
             logger.warning(f"Skipped {len(devices_input) - len(identifiers)} item(s) in 'devices' list due to invalid type or empty string.")
    else:
...
def read_file_list(file_path: str, logger_instance: logging.Logger) -> List[str]:
    """Reads a list of items (one per line) from a file."""
    items: List[str] = []
    logger_instance.debug(f"Attempting to read list from file: '{file_path}'")
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                item = line.strip()
                if item: items.append(item)
        logger_instance.info(f"Read {len(items)} items from file '{file_path}'.")
    except FileNotFoundError:
        logger_instance.error(f"File not found at '{file_path}'.")
    except Exception as e:
...
def collect_and_validate_credentials(
    ...
    for cli_arg_name, dict_key_name in cli_arg_map.items():
        ...
        if cli_value is not None: # Check for None explicitly, as False or 0 are valid values
    ...
    # Iterate over all known keys (required + those in cli_arg_map which might be optional)
